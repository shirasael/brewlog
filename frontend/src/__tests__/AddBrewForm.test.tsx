    ...
    return (
      <div data-testid="digital-clock">
        <input
          ...
          value={
            value
              ? `${value.minute().toString().padStart(2, "0")}:${value
                  .second()
                  .toString()
                  .padStart(2, "0")}`
              : "00:00"
...
describe("AddBrewForm", () => {
  ...
  it("handles brew time input correctly", () => {
    ...
    const timePickerContainer =
      screen.getByTestId("digital-clock").parentElement;
  ...
  it("renders the form with all inputs", () => {
    ...
    expect(
      screen.getByText(/upload an image or take a photo/i),
    ).toBeInTheDocument();

    // Check brew type select and its options
    const brewTypeSelect = screen.getByLabelText(
      /brew type/i,
    ...
    expect(brewTypeSelect.options[brewTypeSelect.options.length - 1].text).toBe(
      "Other",
    );
    ...
    expect(
      screen.getByRole("button", { name: /add brew/i }),
    ).toBeInTheDocument();
